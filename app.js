...
const LEADERSHIP_ROLES = ["charge_nurse","head_nurse","assistant_head_nurse","supervisor","admin"];
...
let stagedDates = []; // for the multi-date picker
...
async function bootstrapSession(){
  ...
  // Load own profile (RLS allows reading own row)
  ...
  // Best-effort: trigger expiry on each session start.
  // RLS allows execute; function is SECURITY DEFINER and idempotent.
...
function switchView(view){
  ...
  if(view === "admin")     { loadUsers(); loadAllAvailability(); }
}
...
function bindStaffForm(){
  $("#btnAddDate").addEventListener("click", () => {
    ...
    if(!v) return;
    if(stagedDates.includes(v)) { $("#afDate").value=""; return; }
    stagedDates.push(v);
    $("#afDate").value = "";
    renderDateChips();
  });
...
async function submitAvailability(e){
  ...
  const base = {
    // user_id, full_name, hospital_email, department, job — all set by DB trigger
...
async function loadMyAvailability(){
  ...
  $$("#myAvailBody [data-cancel]").forEach(b => b.addEventListener("click", () => cancelMine(b.dataset.cancel)));
}
...
// =====================================================================
// LEADERSHIP: list, filter, status actions, email
// =====================================================================
...
async function loadLeadership(){
  ...
  // Refresh expirations defensively
  try { await sb.rpc("expire_past_availability"); } catch(_) {}

  let q = sb.from("overtime_availability").select("*").order("available_date", { ascending: true });
  ...
  body.innerHTML = data.map(r => `
    <tr class="${rowClassForStatus(r.status)}">
      ...
      <td><a href="mailto:${esc(r.hospital_email)}?subject=${encodeURIComponent("Overtime Availability — "+formatDate(r.available_date))}&body=${encodeURIComponent(emailBody(r))}">${esc(r.hospital_email)}</a></td>
      ...
      <td>${statusBadge(r.status)}</td>
      <td class="actions">
        ${actionButton(r, "Contacted")}
        ${actionButton(r, "Assigned")}
        ${actionButton(r, "Cancelled")}
      </td>
    </tr>
  `).join("");
...
function actionButton(r, target){
  if(r.status === target) return "";
  if(r.status === "Expired" || r.status === "Cancelled") return "";
  if(r.status === "Assigned" && target !== "Cancelled") return ""; // once assigned, only cancel
  const cls = target === "Assigned" ? "btn-primary"
            : target === "Cancelled" ? "btn-danger"
            : "btn-ghost";
  return `<button class="btn btn-small ${cls}" data-id="${r.id}" data-set-status="${target}">Mark ${target}</button>`;
}

function emailBody(r){
  return `Dear ${r.full_name || "colleague"},

We saw your overtime availability for ${formatDate(r.available_date)} (${r.shift_preference} shift).
...
async function loadUsers(){
  ...
  const roleOpts = ["staff","charge_nurse","head_nurse","assistant_head_nurse","supervisor","admin"];
  ...
  $$("#usersBody [data-save-id]").forEach(b => b.addEventListener("click", () => saveUser(b.dataset.saveId)));
}
...
async function loadAllAvailability(){
  ...
  $$("#allAvailBody [data-admin-del]").forEach(b => b.addEventListener("click", async () => {
    ...
    const { error } = await sb.from("overtime_availability").delete().eq("id", b.dataset.adminDel);
    if(error) return toast(error.message);
    toast("Deleted.");
    loadAllAvailability();
  }));
...
async function exportCsv(){
  ...
  const { data, error } = await sb
    ...
    .order("available_date", { ascending: false });
  if(error){ toast(error.message); return; }
  if(!data || data.length === 0){ toast("Nothing to export."); return; }

  const headers = ["id","user_id","full_name","hospital_email","current_department","job_title",
                   "available_date","shift_preference","can_float","preferred_departments",
  ...
  a.href = url; a.download = `overtime_availability_${new Date().toISOString().slice(0,10)}.csv`;
